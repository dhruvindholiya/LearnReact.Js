...
class Cards extends Component {
    ...
    handleProduct = (productType) => {
        this.setState({ active: productType })

        let filteredProducts = this.state.Products.filter((product) => product.category === productType)
        if (filteredProducts.length === 0) {
            this.setState({ SortedProducts: this.state.Products })
        } else {
            this.setState({ SortedProducts: filteredProducts })
        }
    }
